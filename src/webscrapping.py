...
#         df.to_csv('attractions_cities.csv', index=False)
        
#         print("Scraping complete. Data saved to 'attractions_cities.csv'.")
...
async def run_scraper():
    ...
    async with httpx.AsyncClient(headers=headers, timeout=httpx.Timeout(30.0)) as client:
        ...
        # Read the city_codes.csv file
        city_codes_df = pd.read_csv('../data/city_codes.csv')
        city_codes_df = city_codes_df.head(10)  # For testing purposes
        
        for _, row in city_codes_df.iterrows():
            city = row['City']
            code = row['Code']
            
            print(f"Scraping {city} attractions...")
            
            # Construct the URL using the city code
            url = f"https://www.tripadvisor.com/Attractions-g{code}.html"
            print(url)
            attractions = await scrape_attractions(url, client)
        ...
        df.to_csv('../data/attractions_cities.csv', index=False)
